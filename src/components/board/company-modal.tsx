...
export function CompanyModal({ open, onClose, projectSlug: _projectSlug, personas }: CompanyModalProps) {
  ...
  }, [open, onClose, editingRole]);

  if (!open) return null;

  function handleSelectRoleForCreate(role: Role) {
    // Open edit form in "create" mode — editingPersona with no id
  ...
  async function handleRegenerateForEdit() {
    ...
    // Find role by roleId first, fall back to matching by slug
    const role = roles.find((r) => r.id === editingPersona.roleId)
      || roles.find((r) => r.slug === editingPersona.role);
  ...
  // Validate skill content: must have valid name in frontmatter
  function validateSkillContent(content: string): { valid: boolean; error?: string; name?: string } {
    ...
    if (frontmatterEnd === -1) {
      return { valid: false, error: "Missing closing --- for frontmatter" };
    }
    ...
    if (!/^[a-z0-9-]+$/.test(name)) {
      return { valid: false, error: "Name: only lowercase letters, numbers, hyphens" };
    }
...
`;
    const newSkill: ClaudeSkillDefinition = {
      name: "new-skill",
      description: "",
      content: template,
    };
  ...
  function updateSkillContent(index: number, content: string) {
    ...
    newSkills[index] = {
      ...newSkills[index],
      content,
      name: validation.name || newSkills[index].name,
    };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
      <div className="w-[1200px] h-[90vh] rounded-2xl flex overflow-hidden bg-[var(--bg-card)] border border-[var(--border-medium)]">
        {/* Sidebar */}
        <div className="w-56 flex flex-col border-r border-[var(--border-subtle)] bg-[var(--bg-primary)]">
          ...
          <div className="p-3 border-t border-[var(--border-subtle)]">
            <button
              onClick={onClose}
              className="w-full px-4 py-2.5 rounded-lg text-sm font-medium text-[var(--text-muted)] hover:bg-white/5 transition-colors"
            >
              Close
            </button>
          </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {roles.map((role) => {
                        ...
                        return (
                          <div
                            key={role.id}
                            className="p-5 rounded-xl border border-[var(--border-medium)] bg-[var(--bg-input)] hover:border-[var(--border-subtle)] transition-colors"
                          >
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div>
                                  ...
                                  <span
                                    ...
                                    style={{
                                      backgroundColor: `color-mix(in srgb, ${role.color} 20%, transparent)`,
                                      color: role.color,
                                    }}
                                  >
                                    {role.slug}
                            ...
                            {/* Claude Code Skills count + Workers */}
                              ...
                              {workers.length > 0 && (
                                <span className="flex items-center gap-1">
                                  {workers.slice(0, 3).map((w) => (
                                    <div
                                      key={w.id}
                                      className="w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold text-white overflow-hidden -ml-1 first:ml-0 border border-[var(--bg-input)]"
                                      style={{ backgroundColor: role.color }}
                                      title={w.name}
                                    >
                                      {w.avatar ? (
                                  ...
                                  {workers.length > 3 && (
                                    <span className="text-[10px] text-[var(--text-muted)]">+{workers.length - 3}</span>
                                  )}
                                </span>
                              )}
                            </div>
                ...
                <>
                  <div className="flex items-center gap-3 px-8 py-5 border-b border-[var(--border-subtle)]">
                    <button
                      ...
                      className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:bg-white/10"
                    >
                    ...
                    <div className="max-w-2xl space-y-6">
                          ...
                          <div className="flex gap-2">
                            {["#6366f1", "#f59e0b", "#10b981", "#ec4899", "#8b5cf6", "#ef4444", "#14b8a6", "#f97316"].map((c) => (
                              <button
                                ...
                                className={`w-8 h-8 rounded-full transition-transform ${roleForm.color === c ? "scale-110 ring-2 ring-white" : ""}`}
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={2}
                          className="w-full px-4 py-3 rounded-lg text-sm bg-[var(--bg-input)] border border-[var(--border-medium)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] outline-none focus:border-[var(--accent-blue)] resize-none"
                        />
                      </div>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <button
                            type="button"
                            onClick={addNewSkill}
                            className="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/10 text-[var(--text-secondary)] hover:bg-white/15 flex items-center gap-1"
                          >
                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                            </svg>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div
                                    ...
                                    onClick={() => setEditingSkillIndex(isExpanded ? null : idx)}
                                  >
                                    <div className="flex items-center gap-2">
                                      <svg
                                      ...
                                      <span className="text-sm font-medium text-[var(--text-primary)]">
                                        /{validation.name || skill.name || "unnamed"}
                                      </span>
                                      {!validation.valid && (
                                    ...
                                    <button
                                      ...
                                      className="w-6 h-6 rounded flex items-center justify-center text-[var(--text-muted)] hover:text-red-400 hover:bg-red-500/10"
                                    >
                                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
Instructions for Claude...`}
                                        rows={12}
                                        className="w-full mt-3 px-3 py-2 rounded-lg text-sm font-mono bg-[var(--bg-primary)] border border-[var(--border-medium)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] outline-none focus:border-[var(--accent-blue)] resize-y"
                                        style={{ minHeight: "200px" }}
                                      />
                                      <p className="text-[10px] text-[var(--text-muted)] mt-2">
                                        Format: YAML frontmatter (name, description, allowed-tools, etc.) + markdown instructions
                                      </p>
                                    </div>
                                  )}
                                </div>
                      ...
                      <div className="flex justify-between pt-4">
                        {editingRole.id ? (
                          <button
                            ...
                            className="px-4 py-2 rounded-lg text-sm font-medium text-red-400 hover:bg-red-500/10"
                          >
                            Delete Role
                          </button>
                        ) : (
                        ...
                        <button
                          ...
                          disabled={!roleForm.title.trim() || savingRole}
                          className="px-6 py-2 rounded-lg text-sm font-semibold text-white bg-[var(--accent-blue)] disabled:opacity-40 hover:opacity-90"
                        >
                ...
                return (
                  <>
                    <div className="flex items-center gap-3 px-8 py-5 border-b border-[var(--border-subtle)]" style={{ borderTopWidth: "3px", borderTopColor: editAccent }}>
                      <button
                        onClick={() => { setTab(editingPersona.id ? "workers" : "roles"); setEditingPersona(null); setSelectedRole(null); }}
                        className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:bg-white/10"
                      >
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                      ...
                      <div className="flex gap-8 max-w-2xl">
                        ...
                        <div className="flex flex-col items-center gap-4">
                          <div className="relative">
                            {avatarUrl ? (
                              <img
                                ...
                                style={{ borderColor: editAccent, opacity: generatingPhase === "avatar" ? 0.4 : 1, transition: "opacity 0.2s" }}
                              />
                            ) : (
                              <div
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={handleRegenerateForEdit}
                              disabled={generating || !!rerolling}
                              className="flex-1 px-3 py-2 rounded-lg text-xs font-medium border border-[var(--border-medium)] text-[var(--text-secondary)] hover:bg-white/5 disabled:opacity-40 flex items-center justify-center gap-1.5"
                            >
                        ...
                        <div className="flex-1 space-y-4">
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {editRole.skillDefinitions.map((skill, idx) => (
                                  <span
                                    ...
                                    style={{
                                      backgroundColor: `color-mix(in srgb, ${editRole.color} 15%, transparent)`,
                          ...
                          <button
                            ...
                            className="w-full px-6 py-3 rounded-lg text-sm font-semibold text-white disabled:opacity-40 hover:opacity-90 transition-opacity"
                            style={{ backgroundColor: editAccent }}
                          >
