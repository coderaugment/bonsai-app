...
export default function WorkersPage() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      {selectedWorker ? (
        <WorkerView worker={selectedWorker} />
      ) : (
        <div className="flex-1 flex items-center justify-center">
...
function WorkerView({ worker }: { worker: Worker }) {
  ...
  // Group by ticket
  const ticketGroups = new Map<string, ActivityItem[]>();
  for (const item of filtered) {
    const key = item.ticketId;
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      ...
      {/* Right: activity feed — the main event */}
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Filter bar */}
        <div
...
function FeedItem({ item }: { item: ActivityItem; workerName: string }) {
